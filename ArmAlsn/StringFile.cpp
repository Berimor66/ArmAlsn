...
// ==================================================================
BOOL CStringFile::Open(LPCTSTR szFile, CFileException *feError, int Unicode)
{
	CFileStatus		rStatus;
	if (Unicode) 
	{
#ifdef _v100
		if(!m_fFile.Open(szFile, CFile::modeRead | CFile::typeText
								| CFile::shareDenyWrite,feError))
#else
		if(!m_fFile.Open(szFile, CFile::modeRead | CFile::typeUnicode 
								| CFile::shareDenyWrite,feError))
#endif
		return FALSE;
	}
...
DWORD CStringFile::GetNextLine(CString &szLine)
{
	...
	//LPWSTR	szBuffer;
	//szBuffer = new WCHAR[m_nBufferSize]; //*sizeof(char)

	char	*szBuffer;
	szBuffer = new char[m_nBufferSize]; //*sizeof(char)
...
//DWORD CStringFile::GetNextLine(LPWSTR szLine, int iLineSize)
	...
	*chTemp = 0;
	...
	while(!bStop)
	{
		...
		for(;m_dwIndex < m_dwRead;m_dwIndex++)
		{
			if((nOut+1) == iLineSize)
			{
				m_szError.Format(L"m_pBuffer переполнение в строке %u (line length over %d chars)",++m_dwLine,iLineSize);
				::MessageBox(NULL,m_szError,L"Warning !",MB_ICONEXCLAMATION|MB_OK);
