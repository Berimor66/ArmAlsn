// DataRowObject.h: объявление CDataRowObject

#pragma once

// создаваемый код 30 мая 2010 г., 23:16

class CDataRowObjectAccessor
{
public:
	LONG m_Serialnumber;
	TCHAR m_ObgectID[51];
...
// потребуется сохранить пароль в другом виде или использовать другую проверку подлинности.
	...
	DEFINE_COMMAND_EX(CDataRowObjectAccessor, L" \
	SELECT \
		`Serial number`, \
		ObgectID, \
		Pk, \
	...
	BEGIN_COLUMN_MAP(CDataRowObjectAccessor)
...
class CDataRowObject : public CCommand<CAccessor<CDataRowObjectAccessor> >
{
public:
	HRESULT OpenAll()
	{
