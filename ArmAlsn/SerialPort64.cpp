...
to maintain a single distribution poINT_PTR for the source code. 
...
#if _MSC_VER >= 1700
BOOL CSerialException::GetErrorMessage(_Out_writes_z_(nMaxError) LPTSTR lpszError, _In_ UINT_PTR nMaxError,	_Out_opt_ PUINT_PTR pnHelpContext)
#else	
BOOL CSerialException::GetErrorMessage(__out_ecount_z(nMaxError) LPTSTR lpszError, __in UINT_PTR nMaxError, __out_opt PUINT_PTR pnHelpContext)
#endif
{
...
void CSerialPort::Open(INT_PTR nPort, DWORD dwBaud, Parity parity, BYTE DataBits, StopBits stopBits, FlowControl fc, BOOL bOverlapped)
{
...
void CSerialPort::GetDefaultConfig(INT_PTR nPort, COMMCONFIG& config)
{
...
void CSerialPort::SetDefaultConfig(INT_PTR nPort, COMMCONFIG& config)
{
  //Create the device name as a string
  CString sPort;
  sPort.Format(_T("COM%d"), nPort);
