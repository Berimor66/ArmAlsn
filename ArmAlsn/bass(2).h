...
typedef struct
{
	wchar_t Version[4];				// version of the data structure
	wchar_t Title[64];					// title of cart audio sequence
	wchar_t Artist[64];				// artist or creator name
	wchar_t CutID[64];					// cut number identification
	wchar_t ClientID[64];				// client identification
	wchar_t Category[64];				// category ID, PSA, NEWS, etc
	wchar_t Classification[64];		// classification or auxiliary key
	wchar_t OutCue[64];				// out cue text
	wchar_t StartDate[10];				// yyyy-mm-dd
	wchar_t StartTime[8];				// hh:mm:ss
	wchar_t EndDate[10];				// yyyy-mm-dd
	wchar_t EndTime[8];				// hh:mm:ss
	wchar_t ProducerAppID[64];			// name of vendor or application
	wchar_t ProducerAppVersion[64];	// version of producer application
	wchar_t UserDef[64];				// user defined text
	DWORD_PTR dwLevelReference;			// sample value for 0 dB reference
	TAG_CART_TIMER PostTimer[8];	// 8 time markers after head
	wchar_t Reserved[276];
	wchar_t URL[1024];					// uniform resource locator
#if defined(__GNUC__) && __GNUC__<3
	wchar_t TagText[0];				// free form text for scripts or tags
#elif 1 // change to 0 if compiler fails the following line
	wchar_t TagText[];					// free form text for scripts or tags
#else
