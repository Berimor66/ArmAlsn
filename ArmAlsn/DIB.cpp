// DIB.cpp - ִכÿ נאבמעû c bmp פאיכאלט http://lib.zabspu.ru/computers/prg/c/mfc/lesson010.htm
...
BOOL CDib::Load( const char *pszFilename )
{
	...
	//DWORD 
		...
		ULONGLONG  dwDibSize;		   //ULONGLONG 
	dwDibSize =
		cf.GetLength() - sizeof( BITMAPFILEHEADER );

	// Attempt to allocate the Dib memory.
	ULONGLONG *pDib;	   //unsigned char 
#pragma warning(push)
#pragma warning(disable:4244)
	pDib = new ULONGLONG [dwDibSize];
#pragma warning(pop)
	if( pDib == NULL )
		return( FALSE );
	...
	try{
		...
		if( cf.Read( &BFH, sizeof( BITMAPFILEHEADER ) )
			...
			cf.Read( pDib, (UINT)dwDibSize ) != (UINT)dwDibSize ){
...
BOOL CDib::Save( const char *pszFilename )
{
	...
	if( !cf.Open((LPCTSTR) pszFilename,
