...
#if _MSC_VER >= 1700
  virtual BOOL GetErrorMessage(_Out_writes_z_(nMaxError) LPTSTR lpszError, _In_ UINT_PTR nMaxError,	_Out_opt_ PUINT_PTR pnHelpContext = NULL);
#else	
  virtual BOOL GetErrorMessage(__out_ecount_z(nMaxError) LPTSTR lpszError, __in UINT_PTR nMaxError, __out_opt PUINT_PTR pnHelpContext = NULL);
#endif
...
//General Methods
  void Open(INT_PTR nPort, DWORD dwBaud = 9600, Parity parity = NoParity, BYTE DataBits = 8, 
...
//Configuration Methods
  ...
  static void GetDefaultConfig(INT_PTR nPort, COMMCONFIG& config);
  void SetConfig(COMMCONFIG& Config);
  static void SetDefaultConfig(INT_PTR nPort, COMMCONFIG& config);
