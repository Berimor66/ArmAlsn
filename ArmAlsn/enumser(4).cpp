...
BOOL CEnumerateSerial::RegQueryValueString(HKEY kKey, LPCTSTR lpValueName, LPTSTR& pszValue)
{
  ...
  DWORD_PTR dwType = 0;
  DWORD_PTR dwDataSize = 0;
  LONG nError = RegQueryValueEx(kKey, lpValueName, NULL,(LPDWORD)&dwType, NULL, (LPDWORD)&dwDataSize); //&
  ...
  DWORD_PTR dwAllocatedSize = dwDataSize + sizeof(TCHAR); //+sizeof(TCHAR) is to allow us to NULL terminate the data if it is not null terminated in the registry
  ...
  pszValue[0] = _T('\0');
  DWORD_PTR dwReturnedSize = dwAllocatedSize;
  nError = RegQueryValueEx(kKey, lpValueName, NULL, (LPDWORD)&dwType, reinterpret_cast<LPBYTE>(pszValue), (LPDWORD)&dwReturnedSize);
  if (nError != ERROR_SUCCESS)
  {
    LocalFree(pszValue);
    pszValue = NULL;
   ...
   INT_PTR nChars = 4096;
    ...
    while (nChars && !bWantStop)
    {
      ...
      if (devices.Allocate(nChars * sizeof(TCHAR)))
      {
        ...
        DWORD_PTR dwChars = QueryDosDevice(NULL, pszDevices, nChars);
        if (dwChars == 0)
        {
...
#endif
{
  ...
  DWORD_PTR dwGuids = 0;
  lpfnSETUPDICLASSGUIDSFROMNAME(_T("Ports"), NULL, 0, (PDWORD)&dwGuids);
  if (dwGuids == 0)
  {
    //Set the error to report
    setupAPI.m_dwError = GetLastError();
  ...
  if (!lpfnSETUPDICLASSGUIDSFROMNAME(_T("Ports"), pGuids, dwGuids, (LPDWORD)&dwGuids))
  {
    //Set the error to report
    setupAPI.m_dwError = GetLastError();
...
#endif
{
  ...
  DWORD_PTR cbNeeded = 0;
  DWORD_PTR dwPorts = 0;
  EnumPorts(NULL, 1, NULL, 0, (LPDWORD)&cbNeeded, (LPDWORD)&dwPorts);
  ...
  if (portsBuffer.Allocate(cbNeeded))
  {
    ...
    bSuccess = EnumPorts(NULL, 1, pPorts, cbNeeded, (LPDWORD)&cbNeeded, (LPDWORD)&dwPorts);
    if (bSuccess)
    {
      PORT_INFO_1* pPortInfo = reinterpret_cast<PORT_INFO_1*>(pPorts);
      for (DWORD_PTR i=0; i<dwPorts; i++)
      {
        //If it looks like "COMX" then
        //add it to the array which will be returned
...
#endif
{
  ...
  if ((lpfnLPCOMDBOPEN != NULL) && (lpfnLPCOMDBCLOSE != NULL) && (lpfnCOMDBGETCURRENTPORTUSAGE != NULL))
  {
    ...
    if (dwComOpen == ERROR_SUCCESS)
    {
      ...
      DWORD_PTR dwMaxPortsReported = 0;
      DWORD_PTR dwPortUsage = lpfnCOMDBGETCURRENTPORTUSAGE(hComDB, NULL, 0, CDB_REPORT_BYTES, (LPDWORD)&dwMaxPortsReported);
      if (dwPortUsage == ERROR_SUCCESS)
      {
        ...
        if (portBytes.Allocate(dwMaxPortsReported))
        {
          ...
          if (lpfnCOMDBGETCURRENTPORTUSAGE(hComDB, pPortBytes, dwMaxPortsReported, CDB_REPORT_BYTES, (LPDWORD)&dwMaxPortsReported) == ERROR_SUCCESS)
          {
...
#endif
{
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("HARDWARE\\DEVICEMAP\\SERIALCOMM"), 0, KEY_QUERY_VALUE, &hSERIALCOMM) == ERROR_SUCCESS)
  {
		...
		DWORD_PTR dwMaxValueNameLen;
		DWORD_PTR dwMaxValueLen;
		DWORD_PTR dwQueryInfo = RegQueryInfoKey(hSERIALCOMM, NULL, NULL, NULL, NULL, NULL, NULL, NULL, (LPDWORD)&dwMaxValueNameLen, (LPDWORD)&dwMaxValueLen, NULL, NULL);
		if (dwQueryInfo == ERROR_SUCCESS)
		{
			DWORD_PTR dwMaxValueNameSizeInChars = dwMaxValueNameLen + 1; //Include space for the NULL terminator
			DWORD_PTR dwMaxValueNameSizeInBytes = dwMaxValueNameSizeInChars * sizeof(TCHAR);
			DWORD_PTR dwMaxValueDataSizeInChars = dwMaxValueLen/sizeof(TCHAR) + 1; //Include space for the NULL terminator
			DWORD_PTR dwMaxValueDataSizeInBytes = dwMaxValueDataSizeInChars * sizeof(TCHAR);
      ...
      if (valueName.Allocate(dwMaxValueNameSizeInBytes) && valueData.Allocate(dwMaxValueDataSizeInBytes))
      {
				...
				DWORD_PTR dwIndex = 0;
				DWORD_PTR dwType;
				DWORD_PTR dwValueNameSize = dwMaxValueNameSizeInChars;
				DWORD_PTR dwDataSize = dwMaxValueDataSizeInBytes;
        ...
        BYTE* byValue = static_cast<BYTE*>(valueData.m_pData);
				LONG nEnum = RegEnumValue(hSERIALCOMM, dwIndex, szValueName, (LPDWORD)&dwValueNameSize, NULL, (LPDWORD)&dwType, byValue, (LPDWORD)&dwDataSize);
            ...
            ports.Add(szPort);
					...
					memset(valueData.m_pData, 0, dwMaxValueDataSizeInBytes);
					++dwIndex;
					nEnum = RegEnumValue(hSERIALCOMM, dwIndex, szValueName, (LPDWORD)&dwValueNameSize, NULL, (LPDWORD)&dwType, byValue, (LPDWORD)&dwDataSize);
				}
      }
      else
		    SetLastError(ERROR_OUTOFMEMORY);
