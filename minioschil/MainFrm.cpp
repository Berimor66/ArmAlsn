...
//static UINT uMessageID = ::RegisterWindowMessage((LPCWSTR)L"MyMessage_TOK");
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	//ON_REGISTERED_MESSAGE(uMessageID, &CMainFrame::OnTok)

	ON_COMMAND_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_WINDOWS_7, &CMainFrame::OnApplicationLook)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_WIN_2000, ID_VIEW_APPLOOK_WINDOWS_7, &CMainFrame::OnUpdateApplicationLook)
	ON_COMMAND_RANGE(ID_WAVEFORM, ID_NORM_FFT, &CMainFrame::OnViewMenuItems)
	ON_UPDATE_COMMAND_UI_RANGE(ID_WAVEFORM, ID_NORM_FFT, &CMainFrame::OnUpdateViewMenuItems)
END_MESSAGE_MAP()
...
CMainFrame::CMainFrame()
{
	...
	theApp.specmodeLook = theApp.GetInt(_T("ViewMenuItems"), ID_WAVEFORM);
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (HIWORD(BASS_GetVersion())!=BASSVERSION) {
		AfxMessageBox(L"Не коректная версия BASS.DLL",0,MB_ICONERROR);
		return 0;
	}
	...
	// установите наглядный диспетчер осциллографа
	OnViewMenuItems(theApp.specmodeLook);
	...
	lstBasicCommands.AddTail(ID_WAVEFORM);
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	cs.cx = SPECWIDTH+2*GetSystemMetrics(SM_CXDLGFRAME);
	cs.cy =	SPECHEIGHT+GetSystemMetrics(SM_CYCAPTION)+2*GetSystemMetrics(SM_CYDLGFRAME);

	//cs.style = WS_POPUPWINDOW|WS_CAPTION|WS_VISIBLE|WS_MINIMIZEBOX;
...
void CMainFrame::OnViewMenuItems(UINT id)
{
	CWaitCursor wait;

	theApp.specmodeLook = id;

	switch (theApp.specmodeLook )
	{
	case ID_WAVEFORM:
		specmode=3;
		break;
	...
	theApp.WriteInt(_T("ViewMenuItems"), theApp.specmodeLook);
}

void CMainFrame::OnUpdateViewMenuItems(CCmdUI* pCmdUI)
{
	pCmdUI->SetRadio(theApp.specmodeLook == pCmdUI->m_nID);
}
